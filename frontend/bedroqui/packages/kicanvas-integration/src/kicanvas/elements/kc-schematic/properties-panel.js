...
export class KCSchematicPropertiesPanelElement extends KCUIElement {
    ...
    setup_events() {
        this.addDisposable(this.viewer.addEventListener(KiCanvasSelectEvent.type, (e) => {
            this.selected_item = e.detail.item;
            this.update();
        }));
        // If a new schematic is loaded, clear the selected item.
        this.addDisposable(this.viewer.addEventListener(KiCanvasLoadEvent.type, (e) => {
            this.selected_item = undefined;
            this.update();
        }));
    }
    render() {
        const header = (name) => html `<kc-ui-property-list-item
                class="label"
                name="${name}"></kc-ui-property-list-item>`;
        const entry = (name, desc, suffix = "") => html `<kc-ui-property-list-item name="${name}">
                ${desc ?? ""} ${suffix}
        ...
        if (!item) {
            ...
            const pins = sorted_by_numeric_strings(item.unit_pins, (pin) => pin.number).map((p) => {
            ...
            entries = html `
                ...
                ${item.unit
                ? entry("Unit", String.fromCharCode("A".charCodeAt(0) + item.unit - 1))
                : ""}
                ...
                ${entry("Units are interchangeable", checkbox(lib.units_interchangable))}
                ${header("Pins")} ${pins}
            `;
        }
            ...
            const pins = sorted_by_numeric_strings(item.pins, (pin) => pin.name).map((p) => {
...
window.customElements.define("kc-schematic-properties-panel", KCSchematicPropertiesPanelElement);
