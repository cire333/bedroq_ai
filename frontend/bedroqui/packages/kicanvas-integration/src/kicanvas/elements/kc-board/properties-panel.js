...
export class KCBoardPropertiesPanelElement extends KCUIElement {
    ...
    render() {
        const header = (name) => html `<kc-ui-property-list-item class="label" name="${name}">
        ...
        if (!this.selected_item) {
            ...
            entries = html `
                ...
                ${entry("Type", itm.attr.through_hole
                ? "through hole"
                : itm.attr.smd
                    ? "smd"
                    : "unspecified")}
                ...
                ${entry("Exclude from position files", checkbox(itm.attr.exclude_from_pos_files))}
                ${entry("Exclude from BOM", checkbox(itm.attr.exclude_from_bom))}
                ${header("Overrides")}
                ${entry("Exempt from courtyard requirement", checkbox(itm.attr.allow_missing_courtyard))}
...
window.customElements.define("kc-board-properties-panel", KCBoardPropertiesPanelElement);
