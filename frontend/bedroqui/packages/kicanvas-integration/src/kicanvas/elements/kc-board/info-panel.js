...
export class KCBoardInfoPanelElement extends KCUIElement {
    ...
    render() {
        ...
        const comments = Object.entries(board.title_block?.comment || {}).map(([k, v]) => entry(`Comment ${k}`, v));
        return html `
            <kc-ui-panel>
                ...
                <kc-ui-panel-body>
                    <kc-ui-property-list>
                        ...
                        ${entry("Thickness", board.general?.thickness ?? 1.6, "mm")}
                        ...
                        ${comments}
                        ${entry("Dimensions", `${board_bbox.w.toFixed(1)} x
                        ...
                        ${entry("Pad to mask clearance", board.setup?.pad_to_mask_clearance ?? 0, "mm")}
                        ${entry("Soldermask min width", board.setup?.solder_mask_min_width ?? 0, "mm")}
                        ${entry("Pad to paste clearance", board.setup?.pad_to_paste_clearance ?? 0, "mm")}
                        ${entry("Pad to paste clearance ratio", board.setup?.pad_to_paste_clearance_ratio ?? 0)}
                        ${entry("Grid origin", `${board.setup?.grid_origin?.x ?? 0}, ${board.setup?.grid_origin?.y ?? 0}`)}
                    </kc-ui-property-list>
                </kc-ui-panel-body>
            </kc-ui-panel>
        `;
