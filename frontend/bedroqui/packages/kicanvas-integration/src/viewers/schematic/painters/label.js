...
export class LabelPainter extends SchematicItemPainter {
    ...
    paint(layer, l) {
        ...
        const pos = schtext.text_pos.add(this.get_schematic_text_offset(l, schtext));
        this.gfx.state.push();
        this.gfx.state.stroke = this.color;
        this.gfx.state.fill = this.color;
        StrokeFont.default().draw(this.gfx, schtext.shown_text, pos, schtext.attributes);
    ...
    get_text_offset(schtext) {
        ...
        return Math.round(schematic_items.DefaultValues.text_offset_ratio *
            schtext.text_size.x);
    }
    get_box_expansion(schtext) {
        // From SCH_LABEL_BASE::GetLabelBoxExpansion
        return Math.round(schematic_items.DefaultValues.label_size_ratio *
            schtext.text_size.y);
    }
    /**
    ...
    get_schematic_text_offset(l, schtext) {
        ...
        const dist = Math.round(this.get_text_offset(schtext) +
...
export class NetLabelPainter extends LabelPainter {
    constructor() {
        super(...arguments);
        this.classes = [schematic_items.NetLabel];
    }
    get color() {
...
export class GlobalLabelPainter extends LabelPainter {
    constructor() {
        super(...arguments);
        this.classes = [schematic_items.GlobalLabel];
    }
    get color() {
...
export class HierarchicalLabelPainter extends LabelPainter {
    constructor() {
        super(...arguments);
        this.classes = [schematic_items.HierarchicalLabel];
    }
    get color() {
    ...
    get_schematic_text_offset(l, schtext) {
        const dist = Math.round(this.get_text_offset(schtext) + schtext.text_width);
        switch (l.at.rotation) {
            case 0:
                return new Vec2(dist, 0);
            case 90:
