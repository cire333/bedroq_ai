...
export class LibSymbolPainter extends SchematicItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [schematic_items.LibSymbol];
    }
    layers_for(item) {
        return [
...
export class SchematicSymbolPainter extends SchematicItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [schematic_items.SchematicSymbol];
    }
    layers_for(item) {
        const layers = [
            LayerNames.interactive,
    ...
    paint(layer, si) {
        ...
        this.gfx.state.push();
        this.gfx.state.matrix = Matrix3.translation(si.at.position.x, si.at.position.y);
 ...
 */
function get_symbol_body_and_pins_bbox(theme, si) {
