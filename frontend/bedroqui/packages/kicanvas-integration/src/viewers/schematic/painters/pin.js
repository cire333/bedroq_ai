...
export class PinPainter extends SchematicItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [schematic_items.PinInstance];
    }
    layers_for(item) {
        return [
    ...
    paint(layer, p) {
        ...
        if (layer.name == LayerNames.symbol_pin ||
    ...
    draw_pin_shape(gfx, pin) {
        const { p0, dir } = PinShapeInternals.stem(pin.position, pin.orientation, pin.def.length);
        PinShapeInternals.draw(gfx, pin.def.type, pin.def.shape, pin.position, p0, dir);
    }
    /**
    ...
    draw_name_and_number(gfx, pin) {
        ...
        const text_margin = 0.6096 * schematic_items.DefaultValues.text_offset_ratio;
        ...
        if (pin_name_offset > 0) {
            ...
            name_placement = hide_pin_names
                ? undefined
                : PinLabelInternals.place_inside(pin_name_offset, name_thickness, pin_length, pin.orientation);
            num_placement = hide_pin_numbers
                ? undefined
                : PinLabelInternals.place_above(text_margin, pin_thickness, num_thickness, pin_length, pin.orientation);
        }
        else {
            // Names are placed above, number are placed below.
            name_placement = hide_pin_names
                ? undefined
                : PinLabelInternals.place_above(text_margin, pin_thickness, name_thickness, pin_length, pin.orientation);
            num_placement = hide_pin_numbers
                ? undefined
                : PinLabelInternals.place_below(text_margin, pin_thickness, name_thickness, pin_length, pin.orientation);
        }
        if (name_placement) {
            PinLabelInternals.draw(gfx, name, pin.position, name_placement, def.name.effects, gfx.state.stroke);
        }
        if (num_placement) {
            PinLabelInternals.draw(gfx, number, pin.position, num_placement, def.number.effects, gfx.state.stroke);
        }
    }
}
/**
...
export const PinShapeInternals = {
    stem(position, orientation, length) {
        const p0 = new Vec2();
        const dir = new Vec2();
        switch (orientation) {
            case "up":
    ...
    draw(gfx, electrical_type, shape, position, p0, dir) {
        ...
        switch (shape) {
            ...
            case "non_logic":
                gfx.line([p0, position]);
                gfx.line([
                    p0.sub(new Vec2(dir.x + dir.y, dir.y - dir.x).multiply(radius)),
                    p0.add(new Vec2(dir.x + dir.y, dir.y - dir.x).multiply(radius)),
                ]);
                gfx.line([
                    p0.sub(new Vec2(dir.x - dir.y, dir.y + dir.x).multiply(radius)),
                    p0.add(new Vec2(dir.x - dir.y, dir.y + dir.x).multiply(radius)),
...
export const PinLabelInternals = {
    ...
    place_above(text_margin, pin_thickness, text_thickness, pin_length, orientation) {
        const offset = new Vec2(pin_length / 2, -(text_margin + pin_thickness / 2 + text_thickness / 2));
    ...
    draw(gfx, text, position, placement, effects, color) {
        ...
        StrokeFont.default().draw(gfx, edatext.shown_text, edatext.text_pos, edatext.attributes);
    },
};
