...
import { Arc, Circle, Color, Polygon, Polyline, } from "../../graphics";
...
import { GlobalLabelPainter, HierarchicalLabelPainter, NetLabelPainter, } from "./painters/label";
...
class RectanglePainter extends SchematicItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [schematic_items.Rectangle];
    }
    layers_for(item) {
    ...
    #stroke(layer, r, pts) {
        ...
        this.gfx.line(new Polyline(pts, r.stroke?.width || this.gfx.state.stroke_width, color));
    }
    #fill(layer, r, pts) {
...
class PolylinePainter extends SchematicItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [schematic_items.Polyline];
    }
    layers_for(item) {
        return [LayerNames.notes];
    }
...
class WirePainter extends SchematicItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [schematic_items.Wire];
    }
    layers_for(item) {
        return [LayerNames.wire];
    }
    paint(layer, w) {
        this.gfx.line(new Polyline(w.pts, this.gfx.state.stroke_width, this.theme.wire));
    }
}
class BusPainter extends SchematicItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [schematic_items.Bus];
    }
    layers_for(item) {
        return [LayerNames.wire];
    }
    paint(layer, w) {
        this.gfx.line(new Polyline(w.pts, schematic_items.DefaultValues.bus_width, this.theme.bus));
    }
}
class BusEntryPainter extends SchematicItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [schematic_items.BusEntry];
    }
    layers_for(item) {
        return [LayerNames.junction];
    }
    paint(layer, be) {
        this.gfx.line(new Polyline([be.at.position, be.at.position.add(be.size)], schematic_items.DefaultValues.wire_width, this.theme.wire));
    }
}
class CirclePainter extends SchematicItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [schematic_items.Circle];
    }
    layers_for(item) {
        return [LayerNames.notes];
    }
    ...
    #stroke(layer, c) {
        ...
        this.gfx.arc(new Arc(c.center, c.radius, new Angle(0), new Angle(Math.PI * 2), width, color));
    }
    #fill(layer, c) {
...
class ArcPainter extends SchematicItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [schematic_items.Arc];
    }
    layers_for(item) {
        return [LayerNames.notes];
    }
    paint(layer, a) {
        const arc = MathArc.from_three_points(a.start, a.mid, a.end, a.stroke?.width);
    ...
    #stroke(layer, a, arc) {
        ...
        this.gfx.arc(new Arc(arc.center, arc.radius, arc.start_angle, arc.end_angle, width, color));
    }
    #fill(layer, a, arc) {
...
class JunctionPainter extends SchematicItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [schematic_items.Junction];
    }
    layers_for(item) {
        return [LayerNames.junction];
    }
    paint(layer, j) {
        ...
        this.gfx.circle(new Circle(j.at.position, (j.diameter || 1) / 2, color));
    }
}
class NoConnectPainter extends SchematicItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [schematic_items.NoConnect];
    }
    layers_for(item) {
        return [LayerNames.junction];
    }
    paint(layer, nc) {
        ...
        this.gfx.state.push();
        this.gfx.state.matrix.translate_self(nc.at.position.x, nc.at.position.y);
        this.gfx.line(new Polyline([new Vec2(-size, -size), new Vec2(size, size)], width, color));
        this.gfx.line(new Polyline([new Vec2(size, -size), new Vec2(-size, size)], width, color));
        this.gfx.state.pop();
    }
}
class TextPainter extends SchematicItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [schematic_items.Text];
    }
    layers_for(item) {
        return [LayerNames.notes];
    }
    paint(layer, t) {
        ...
        this.gfx.state.push();
        StrokeFont.default().draw(this.gfx, schtext.shown_text, schtext.text_pos, schtext.attributes);
        this.gfx.state.pop();
    }
}
class PropertyPainter extends SchematicItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [schematic_items.Property];
    }
    layers_for(item) {
    ...
    paint(layer, p) {
        ...
        schfield.attributes.stroke_width =
            schfield.get_effective_text_thickness(schematic_items.DefaultValues.line_width * 10000);
        ...
        if (layer.name == LayerNames.interactive) {
            ...
            this.gfx.state.push();
            StrokeFont.default().draw(this.gfx, schfield.shown_text, pos, schfield.attributes);
            this.gfx.state.pop();
        }
    }
}
class LibTextPainter extends SchematicItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [schematic_items.LibText];
    }
    layers_for(item) {
    ...
    paint(layer, lt) {
        ...
        libtext.attributes.color = this.dim_if_needed(this.theme.component_outline);
        ...
        StrokeFont.default().draw(this.gfx, libtext.shown_text, pos, libtext.attributes);
        // this.paint_debug(bbox);
        this.gfx.state.pop();
    }
    paint_debug(bbox) {
        this.gfx.line(Polyline.from_BBox(bbox.scale(1 / 10000), 0.127, new Color(0, 0, 1, 1)));
        this.gfx.circle(new Circle(bbox.center.multiply(1 / 10000), 0.2, new Color(0, 1, 0, 1)));
    }
}
class SchematicSheetPainter extends SchematicItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [schematic_items.SchematicSheet];
    }
    layers_for(item) {
        return [
    ...
    paint(layer, ss) {
        ...
        const bbox = new BBox(ss.at.position.x, ss.at.position.y, ss.size.x, ss.size.y);
        if (layer.name == LayerNames.interactive) {
        ...
        if (layer.name == LayerNames.symbol_foreground) {
            this.gfx.line(Polyline.from_BBox(bbox, this.gfx.state.stroke_width, outline_color));
        }
