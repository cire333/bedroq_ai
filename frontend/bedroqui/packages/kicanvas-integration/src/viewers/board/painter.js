...
import { CopperVirtualLayerNames, CopperLayerNames, LayerNames, copper_layers_between, virtual_layer_for, } from "./layers";
class BoardItemPainter extends ItemPainter {
    get theme() {
...
class LinePainter extends BoardItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [board_items.GrLine, board_items.FpLine];
    }
    layers_for(item) {
        return [item.layer];
    }
...
class RectPainter extends BoardItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [board_items.GrRect, board_items.FpRect];
    }
    layers_for(item) {
        return [item.layer];
    }
...
class PolyPainter extends BoardItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [board_items.Poly, board_items.GrPoly, board_items.FpPoly];
    }
    layers_for(item) {
        return [item.layer];
    }
...
class ArcPainter extends BoardItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [board_items.GrArc, board_items.FpArc];
    }
    layers_for(item) {
        return [item.layer];
    }
...
class CirclePainter extends BoardItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [board_items.GrCircle, board_items.FpCircle];
    }
    layers_for(item) {
        return [item.layer];
    }
    paint(layer, c) {
        ...
        if (this.isFillValid(c.fill)) {
            this.gfx.circle(new Circle(arc.center, arc.radius + (c.width ?? 0), color));
        }
        else {
            const points = arc.to_polyline();
...
class TraceSegmentPainter extends BoardItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [board_items.LineSegment];
    }
    layers_for(item) {
        return [item.layer];
    }
...
class TraceArcPainter extends BoardItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [board_items.ArcSegment];
    }
    layers_for(item) {
        return [item.layer];
    }
...
class ViaPainter extends BoardItemPainter {
    ...
    layers_for(v) {
        if (v.layers) {
            ...
            const layers = [];
            for (const cu_layer of copper_layers_between(v.layers[0], v.layers[1])) {
                layers.push(virtual_layer_for(cu_layer, CopperVirtualLayerNames.bb_via_holes));
                layers.push(virtual_layer_for(cu_layer, CopperVirtualLayerNames.bb_via_hole_walls));
            }
            return layers;
        }
        else {
    ...
    paint(layer, v) {
        ...
        if (layer.name.endsWith("HoleWalls") ||
            ...
            if ((v.type == "blind" || v.type == "micro") && v.layers) {
                this.gfx.arc(v.at.position, v.size / 2 - v.size / 8, Angle.from_degrees(180 + 70), Angle.from_degrees(360 - 70), v.size / 4, layer.layer_set.by_name(v.layers[0])?.color ??
                    Color.transparent_black);
                this.gfx.arc(v.at.position, v.size / 2 - v.size / 8, Angle.from_degrees(70), Angle.from_degrees(180 - 70), v.size / 4, layer.layer_set.by_name(v.layers[1])?.color ??
                    Color.transparent_black);
            }
        }
    }
...
class PadPainter extends BoardItemPainter {
    ...
    paint(layer, pad) {
        ...
        if (is_hole_layer && pad.drill != null) {
            ...
            if (pad.drill?.offset) {
                this.gfx.state.matrix.translate_self(pad.drill.offset.x, pad.drill.offset.y);
            }
            switch (shape) {
                case "circle":
                ...
                case "trapezoid":
                    ...
                    // and a polyline.
                    {
                        const rounding = Math.min(pad.size.x, pad.size.y) *
                        ...
                        this.gfx.polygon(new Polygon(rect_points, color));
                        this.gfx.line(new Polyline([...rect_points, rect_points[0]], rounding * 2, color));
                ...
                case "oval":
                    {
                        ...
                        if (pad_start.equals(pad_end)) {
                            this.gfx.circle(new Circle(pad_pos, half_width, color));
                        }
                        else {
                            this.gfx.line(new Polyline([pad_start, pad_end], half_width * 2, color));
                        }
                    }
                    break;
                default:
...
class GrTextPainter extends BoardItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [board_items.GrText];
    }
    layers_for(t) {
        return [t.layer.name];
    }
...
class FpTextPainter extends BoardItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [board_items.FpText];
    }
    layers_for(t) {
        if (t.hide) {
            return [];
...
class DimensionPainter extends BoardItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [board_items.Dimension];
    }
    layers_for(d) {
        return [d.layer];
    }
    ...
    paint_center(layer, d) {
        ...
        this.gfx.line([d.start.sub(arm), d.start.add(arm)], thickness, layer.color);
    ...
    paint_radial(layer, d) {
        ...
        // Cross shape
        this.gfx.line([center.sub(center_arm), center.add(center_arm)], thickness, layer.color);
        ...
        const textbox_pt = text_bbox.intersect_segment(line_segs[1], line_segs[2]);
        if (textbox_pt) {
        ...
        const arrow_end_neg = inv_radial_angle
            .sub(arrow_angle)
            .rotate_point(arrow_seg);
        this.gfx.line([d.end.add(arrow_end_neg), d.end, d.end.add(arrow_end_pos)], thickness, layer.color);
        // Text
    ...
    paint_leader(layer, d) {
        ...
        const text_bbox = text
            ...
            .scale(1 / 10000);
        const start = d.start.add(d.end.sub(d.start).resize(d.style.extension_offset));
        ...
        if (d.style.text_frame == 1) {
            this.gfx.line(Polyline.from_BBox(text_bbox, thickness, layer.color));
        }
        if (d.style.text_frame == 2) {
            const radius = text_bbox.w / 2 -
                text.get_effective_text_thickness() / 10000 / 2;
            this.gfx.arc(text_bbox.center, radius, Angle.from_degrees(0), Angle.from_degrees(360), thickness, layer.color);
        }
        // Arrows
        ...
        const arrow_end_neg = inv_radial_angle
            .sub(arrow_angle)
            .rotate_point(arrow_seg);
        this.gfx.line([start.add(arrow_end_neg), start, start.add(arrow_end_pos)], thickness, layer.color);
        // Text
        this.paint_text(text);
    }
    paint_linear(layer, d) {
        ...
        // Draw extensions
        const extension_height = Math.abs(d.height) -
    ...
    paint_text(text) {
        this.gfx.state.push();
        StrokeFont.default().draw(this.gfx, text.shown_text, text.text_pos, text.attributes);
        this.gfx.state.pop();
    }
}
class FootprintPainter extends BoardItemPainter {
    constructor() {
        super(...arguments);
        this.classes = [board_items.Footprint];
    }
    layers_for(fp) {
    ...
    paint(layer, fp) {
        const matrix = Matrix3.translation(fp.at.position.x, fp.at.position.y).rotate_self(Angle.deg_to_rad(fp.at.rotation));
        this.gfx.state.push();
        this.gfx.state.multiply(matrix);
        for (const item of fp.items()) {
            const item_layers = this.view_painter.layers_for(item);
            if (layer.name == ViewLayerNames.overlay ||
