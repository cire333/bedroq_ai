...
export class ShaderProgram {
    static #shader_cache = new WeakMap();
    /**
    ...
    #discover_uniforms() {
        this.uniforms = {};
        for (let u_n = 0; u_n <
            this.gl.getProgramParameter(this.program, this.gl.ACTIVE_UNIFORMS); u_n++) {
            const info = this.gl.getActiveUniform(this.program, u_n);
            if (info == null) {
                throw new Error(`Could not get uniform info for uniform number ${u_n} for program ${this.program}`);
            }
            const location = this.gl.getUniformLocation(this.program, info.name);
            if (location == null) {
                throw new Error(`Could not get uniform location for uniform number ${u_n} for program ${this.program}`);
            }
            this[info.name] = this.uniforms[info.name] = new Uniform(this.gl, info.name, location, info.type);
        }
    }
    #discover_attribs() {
        this.attribs = {};
        for (let a_n = 0; a_n <
            this.gl.getProgramParameter(this.program, this.gl.ACTIVE_ATTRIBUTES); a_n++) {
            const info = this.gl.getActiveAttrib(this.program, a_n);
            if (info == null) {
                throw new Error(`Could not get attribute info for attribute number ${a_n} for program ${this.program}`);
            }
