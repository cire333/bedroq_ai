...
import { ShaderProgram, VertexArray } from "./helpers";
...
class Tesselator {
    ...
    static { this.vertices_per_quad = 2 * 3; }
    /**
    ...
    static tesselate_polyline(polyline) {
        ...
        for (let segment_num = 1; segment_num < points.length; segment_num++) {
            ...
            this.populate_color_data(color_data, color, vertex_index * 4, this.vertices_per_quad * 4);
    ...
    static tesselate_circles(circles) {
        ...
        for (let i = 0; i < circles.length; i++) {
            ...
            position_data.set(this.quad_to_triangles(quad), vertex_index * 2);
            cap_data.set(Array(this.vertices_per_quad).fill(cap_region), vertex_index);
            this.populate_color_data(color_data, c.color, vertex_index * 4, this.vertices_per_quad * 4);
            vertex_index += this.vertices_per_quad;
        }
        return {
            position_array: position_data.slice(0, vertex_index * 2),
...
export class CircleSet {
    ...
    static async load_shader(gl) {
        ...
        this.shader = await ShaderProgram.load(gl, "polyline", polyline_vert_shader_src, polyline_frag_shader_src);
    }
    /**
...
export class PolygonSet {
     ...
     */
    static async load_shader(gl) {
        this.shader = await ShaderProgram.load(gl, "polygon", polygon_vert_shader_src, polygon_frag_shader_src);
    }
    /**
    ...
    set(polygons) {
        ...
        for (const polygon of polygons) {
            ...
            Tesselator.populate_color_data(color_data, polygon.color, color_data_idx, polygon_vertex_count * 4);
