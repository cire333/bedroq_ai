...
export class EDAText {
    ...
    get_text_box(line, invert_y) {
        ...
        let extents = font.get_line_extents(text, font_size, thickness, bold, italic);
        ...
        if (this.multiline && !line && strings.length) {
            for (const line of strings.slice(1)) {
                extents = font.get_line_extents(line, font_size, thickness, bold, italic);
                text_size.x = Math.max(text_size.x, extents.x);
            }
            text_size.y += Math.round((strings.length - 1) * font.get_interline(font_size.y));
        }
        bbox.w = text_size.x;
        bbox.h = text_size.y;
