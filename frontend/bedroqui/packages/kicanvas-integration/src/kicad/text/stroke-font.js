...
export class StrokeFont extends Font {
    ...
    static { this.underline_position_factor = -0.16; }
    ...
    #load_glyph(idx) {
        const data = newstroke.glyph_data[idx];
        if (is_string(data)) {
    ...
    get_line_extents(text, size, thickness, bold, italic) {
        const extents = super.get_line_extents(text, size, thickness, bold, italic);
    ...
    get_text_as_glyphs(text, size, position, angle, mirror, origin, style) {
        ...
        for (const c of text) {
            switch (c) {
                case "\t":
                    {
                        // tabs are 4 spaces.
                        const char_tab_width = Math.round(glyph_size.x * tab_width);
                ...
                default:
                    {
                        ...
                        glyphs.push(source.transform(glyph_size, cursor, tilt, angle, mirror, origin));
                        if (tilt) {
        ...
        if (style.overbar) {
            ...
            has_bar = true;
            bar_offset.y = this.compute_underline_vertical_position(glyph_size.y);
        }
        ...
        if (has_bar) {
            ...
            const bar_start = new Vec2(position.x + bar_offset.x + bar_trim, cursor.y - bar_offset.y);
            const bar_end = new Vec2(cursor.x + bar_offset.x - bar_trim, cursor.y - bar_offset.y);
            const bar_glyph = new StrokeGlyph([[bar_start, bar_end]], BBox.from_points([bar_start, bar_end]));
            glyphs.push(bar_glyph.transform(new Vec2(1, 1), new Vec2(0, 0), 0, angle, mirror, origin));
        }
        const bbox = new BBox();
        bbox.start = position;
        bbox.end = new Vec2(cursor.x + bar_offset.x - Math.round(glyph_size.x * inter_char), cursor.y +
            Math.max(glyph_size.y, bar_offset.y * StrokeFont.overbar_position_factor));
        return {
            bbox: bbox,
            glyphs: glyphs,
 ...
 *
 */
function decode_glyph(glyph_data) {
    let start_x = 0;
    let end_x = 0;
    let width = 0;
    let min_y = 0;
    ...
    for (let i = 0; i < glyph_data.length; i += 2) {
        ...
        if (i < 2) {
            ...
            const point = new Vec2(coord[0] * StrokeFont.font_scale - start_x, (coord[1] + StrokeFont.font_offset) * StrokeFont.font_scale);
