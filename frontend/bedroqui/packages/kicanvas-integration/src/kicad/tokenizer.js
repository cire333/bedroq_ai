...
(function (State) {
    ...
    State[State["atom"] = 3] = "atom";
    State[State["hex"] = 4] = "hex";
})(State || (State = {}));
    ...
    for (let i = 0; i < input.length + 1; i++) {
        ...
        if (state == State.none) {
            if (c === "(") {
                ...
                throw new Error(`Unexpected character at index ${i}: ${c}\nContext: ${error_context(input, i)}`);
            }
        }
            ...
            if (is_atom(c)) {
                ...
                throw new Error(`Unexpected character while tokenizing atom at index ${i}: ${c}\nContext: ${error_context(input, i)}`);
            }
        }
            ...
            if (c === "." || is_digit(c)) {
                ...
                yield new Token(Token.NUMBER, parseFloat(input.substring(start_idx, i)));
                state = State.none;
                if (c === ")") {
                    yield close_token;
                }
                ...
                throw new Error(`Unexpected character at index ${i}: ${c}, expected numeric.\nContext: ${error_context(input, i)}`);
            }
        }
            ...
            if (is_digit(c) ||
                ...
                throw new Error(`Unexpected character at index ${i}: ${c}, expected hexadecimal.\nContext: ${error_context(input, i)}`);
            }
        }
        else if (state == State.string) {
            if (!escaping && c === '"') {
                yield new Token(Token.STRING, input
            ...
            throw new Error(`Unknown tokenizer state ${state}\nContext: ${error_context(input, i)}`);
        }
    }
}
