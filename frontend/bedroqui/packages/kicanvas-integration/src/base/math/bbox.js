...
export class BBox {
    ...
    static combine(boxes, context) {
        ...
        if (min_x == Number.POSITIVE_INFINITY ||
    ...
    scale(s) {
        return BBox.from_points([this.start.multiply(s), this.end.multiply(s)], this.context);
    }
    /**
    ...
    intersect_segment(a, b) {
        ...
        for (const seg of [left, right, top, bottom]) {
            ...
            if (intersection.sub(start).squared_magnitude <
