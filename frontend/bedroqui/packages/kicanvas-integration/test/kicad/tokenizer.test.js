...
function assert_tokens(parsed_tokens, expected_tokens) {
    for (const expected of expected_tokens) {
        ...
        assert.equal(parsed?.type, expected[0], `Expected token ${expected[0].description}: ${expected[1]} found ${parsed?.type.description}: ${parsed?.value}.`);
        assert.equal(parsed?.value, expected[1], `Expected token ${expected[0].description}: ${expected[1]} found ${parsed?.type.description}: ${parsed?.value}.`);
    }
}
suite("kicad.tokenizer.tokenize(): s-expression tokenizer", function () {
    ...
    test("with strings", function () {
        ...
        tokens = tokenizer.tokenize(`descr "Pololu Breakout 16-pin 15.2x20.3mm 0.6x0.8\\\\"`);
        assert_tokens(tokens, [
            [ATOM, "descr"],
    ...
    test("with base64", function () {
        const tokens = tokenizer.tokenize("(6x84OPr6+j7y 0+6li2sAAAAASU/VORK5CYII=)");
        assert_tokens(tokens, [
            OPEN_TOKEN,
            [ATOM, "6x84OPr6+j7y"],
    ...
    test("with embedded data containing pipes", function () {
        const tokens = tokenizer.tokenize("(data |KLUv/aCvzgcAAAAQiVBORw0KGgoAAAANSUhEUgAABiAAAANoCAYAAABJLCIrAAAABHNCSVQICAgI)");
        assert_tokens(tokens, [
            OPEN_TOKEN,
            [ATOM, "data"],
            [
