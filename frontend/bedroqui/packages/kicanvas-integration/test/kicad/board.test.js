...
suite("kicad.board.KicadPCB(): board parsing", function () {
    ...
    test("with title block and properties", function () {
        ...
        const text = pcb.drawings[0];
        assert.equal(text.shown_text, "hello var 1 value");
        const fp = pcb.footprints[0];
        assert.equal(fp.drawings[2].shown_text, "F.Cu An extra property");
        assert.equal(fp.drawings[3].shown_text, "FID1");
    });
    ...
    test("with locked gr_text", function () {
        const pcb = new board.KicadPCB("text-locked.kicad_pcb", text_locked_pcb_src);
    ...
    test("with locked fp_text", function () {
        const pcb = new board.KicadPCB("footprint-text-locked.kicad_pcb", footprint_text_locked_pcb_src);
    ...
    test("with dimensions", function () {
        ...
        assert.deepInclude(pcb.drawings[0].gr_text, {
            text: "preval (mm)post",
...
suite("kicad.board.Footprint()", function () {
    test("with graphics", function () {
        const pcb = new board.KicadPCB("test.kicad_pcb", footprint_graphics_pcb_src);
        const fp = pcb.footprints[0];
        ...
        const [text_ref, text_value, text_2, text_3, text_1, text_ref_fab, line, rect, arc, circle, poly,] = fp.drawings;
    ...
    test("with pads", function () {
        const pcb = new board.KicadPCB("test.kicad_pcb", footprint_pads_pcb_src);
