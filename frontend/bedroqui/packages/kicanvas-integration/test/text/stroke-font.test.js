...
import { StrokeFont, TextAttributes, TextStyle, } from "../../src/kicad/text";
...
suite("text.stroke_font.StrokeFont()", function () {
    test(".get_glyph()", function () {
        ...
        assert.closeTo(neg.strokes[0][0].x, 0.23809523809523814, Number.EPSILON);
        assert.closeTo(neg.strokes[0][0].y, -0.42857142857142855, Number.EPSILON);
    ...
    test(".compute_overbar_vertical_position()", function () {
        assert.closeTo(font.compute_overbar_vertical_position(12700), 17780, 0.5);
    ...
    test(".get_text_as_glyphs()", function () {
        ...
        const { bbox, glyphs, cursor } = font.get_text_as_glyphs(text, size, pos, angle, mirror, origin, style);
    ...
    test(".get_line_extents()", function () {
        let limits = font.get_line_extents("hello world", new Vec2(12700, 12700), 1588, false, false);
        assert.closeTo(limits.x, 107019, 0.5);
        assert.closeTo(limits.y, 16670, 0.5);
        limits = font.get_line_extents("hello world", new Vec2(12700, 12700), 2540, true, false);
        assert.closeTo(limits.x, 109399, 0.5);
        assert.closeTo(limits.y, 19050, 0.5);
    });
    test(".get_line_extents() with markup", function () {
        let limits = font.get_line_extents("hello ^{world} 1 2 3", new Vec2(12700, 12700), 1588, false, false);
        assert.closeTo(limits.x, 150924, 0.5);
        assert.closeTo(limits.y, 16670, 0.5);
        limits = font.get_line_extents("hello _{world} 1 2 3", new Vec2(12700, 12700), 1588, false, false);
        assert.closeTo(limits.x, 150924, 0.5);
        assert.closeTo(limits.y, 16670, 0.5);
        limits = font.get_line_extents("hello ~{world} 1 2 3", new Vec2(12700, 12700), 1588, false, false);
        assert.closeTo(limits.x, 166164, 0.5);
        assert.closeTo(limits.y, 28862, 0.5);
        limits = font.get_line_extents("hello ^{world} _{1 2 3}", new Vec2(12700, 12700), 1588, false, false);
...
const assert_vec = (v, x, y, delta = 0.5) => {
    if (!v) {
