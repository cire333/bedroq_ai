...
suite("dom.templates", function () {
    test("no interpolated values", function () {
        ...
        assert.dom.equal(html `<div attr="value">
                hello!
                <p>nested</p>
            </div>`, `<div attr="value">hello!<p>nested</p></div>`);
    });
    test("content interpolation with primitives", function () {
        assert.dom.equal(html `<div>hello, ${"name"}!</div>`, `<div>hello, name!</div>`);
        assert.dom.equal(html `<div>hello, ${1}!</div>`, `<div>hello, 1!</div>`);
        assert.dom.equal(html `<div>hello, ${null}!</div>`, `<div>hello, !</div>`);
        assert.dom.equal(html `<div>hello, ${true}!</div>`, `<div>hello, true!</div>`);
    });
    test("content interpolation with primitive arrays", function () {
        assert.dom.equal(html `<div>hello, ${["name", 1, null, false]}!</div>`, `<div>hello, name1false!</div>`);
    });
    test("content interpolation with elements", function () {
        const name_elm = html `<p>name</p>`;
    ...
    test("content interpolation with multiple elements", function () {
        const child1 = html `<p>name</p>`;
        const child2 = html `<p>last name</p>`;
        const elm = html `<div>hello, ${[child1, child2]}!</div>`;
        child1.title = "meep";
        child2.title = "moop";
        assert.dom.equal(elm, `<div>hello, <p title="meep">name</p> <p title="moop">last name</p>!</div>`);
    ...
    test("content interpolation with dangerous text", function () {
        assert.dom.equal(html `<div>hello, ${"<p>evil</p>"}!</div>`, `<div>hello, &lt;p&gt;evil&lt;/p&gt;!</div>`);
    });
    test("attribute interpolation with primitives", function () {
        assert.dom.equal(html `<div
                a="${"a"}"
                b="${1}"
                c="${true}"
                d="${false}"
    ...
    test("mixed attribute interpolation", function () {
        assert.dom.equal(html `<div a="hello ${"a"}" b="${1} world" c="a ${"b"} c"></div>`, `<div
                a="hello a"
                b="1 world"
                c="a b c"></div>`);
    });
